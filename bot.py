import loggingimport osimport randomimport re # We need to import 're' for parsing the button textfrom telegram import Update, Bot, InlineKeyboardButton, InlineKeyboardMarkupfrom telegram.ext import (Application,CommandHandler,ContextTypes,filters,MessageHandler,CallbackQueryHandler,JobQueue # <-- Import JobQueue)--- Configuration ---Hardcoded as requested by the user.WARNING: Do not share this file. Your token is secret.TELEGRAM_TOKEN = "7390844050:AAEB5vpDzhRelx0sf_CUGwMgLXd4ZBf61ks"ADMIN_USER_ID = 6837532865How often to send a prediction (in seconds)POST_INTERVAL = 60  # 1 minuteURL for the image you want to send with predictionsPREDICTION_IMAGE_URL = "https://envs.sh/JN2.jpg"Emojis for the reaction buttonsREACTIONS = ["😍", "😢", "🤩", "🤑", "🧡", "💸"]--- NEW: 8 REAL TRENDS FROM SCREENSHOTS ---All old trend logic is GONE. This is the new brain.I have converted "Big" to "Big 🔴" and "Small" to "Small 🟢"B = "Big 🔴"S = "Small 🟢"These are the 8 trends you provided, in orderReading from top (newest) to bottom (oldest)PREDEFINED_TRENDS = {"Trend 1": [S, S, B, S, S, S, B, B, B, B],"Trend 2": [S, B, S, B, B, B, S, B, B, B],"Trend 3": [B, B, S, B, S, B, S, S, B, B],"Trend 4": [B, B, B, S, B, B, S, S, B, B],"Trend 5": [S, B, S, B, S, S, B, S, S, S],"Trend 6": [B, B, S, S, S, S, S, B, B, S],"Trend 7": [B, S, S, B, B, B, S, B, S, B],"Trend 8": [S, S, S, B, S, B, S, B, S, S]}---------------------Enable logginglogging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)logger = logging.getLogger(name)This bot uses context.application.bot_data to store statesbot_data['channel_states'] will be our dictionaryasync def generate_new_trend_logic(context: ContextTypes.DEFAULT_TYPE, chat_id: int):"""This is no longer a job. It's a helper function thatselects one of the 8 predefined trends and sets the queue."""if 'channel_states' not in context.application.bot_data or chat_id not in context.application.bot_data['channel_states']:logger.warning(f"generate_new_trend_logic called for {chat_id} but state not found.")returnstate = context.application.bot_data['channel_states'][chat_id]
...
async def send_prediction_job(context: ContextTypes.DEFAULT_TYPE):"""This job runs every 1 minute.It takes one prediction from the queue and posts it.If the queue is empty, it generates a new trend."""chat_id = context.job.chat_idif 'channel_states' not in context.application.bot_data or chat_id not in context.application.bot_data['channel_states']:
...
async def reaction_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):"""This is the new function that handles when a user clicks a reaction button."""query = update.callback_query# Immediately answer the button press, so the user's app doesn't show a loading iconawait query.answer()clicked_emoji = query.data
...
def stop_all_jobs_for_chat(context: ContextTypes.DEFAULT_TYPE, chat_id: int):"""Helper function to stop and remove all jobs and state for a chat."""bot_data = context.application.bot_data['channel_states']if chat_id in bot_data:state = bot_data[chat_id]    # --- REMOVED trend_job ---
    ...
    return True
return False
--- Admin Command Handlers (Must be run in DM) ---admin_filter = filters.User(user_id=ADMIN_USER_ID)async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):"""Sends a welcome message to the admin."""welcome_text = ("Welcome, Admin! (v2.7.1)\n\n""This version fixes the SyntaxError from my last message.\n""It now uses the 8 real 10-step trends you provided.\n""It will post one step every minute, then automatically start a new trend.\n\n""How to Use:\n""/autotrade on @mychannel\n""/autotrade off @mychannel")await update.message.reply_text(welcome_text, parse_mode='Markdown')async def autotrade_command(update: Update, context: ContextTypes.DEFAULT_TYPE):"""Handles the /autotrade on/off <channel> commands."""try:command = context.args[0].lower()target_channel = context.args[1]except (IndexError, TypeError):await update.message.reply_text("Usage: /autotrade <on/off> <@channel_username or channel_id>", parse_mode='Markdown')return# Normalize channel ID
try:
...
async def unauthorized_user_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):"""Replies to any non-admin who tries to use this bot."""await update.message.reply_text("⛔ You are not authorized to use this bot.")def main():"""Run the bot."""if not TELEGRAM_TOKEN:logging.critical("TELEGRAM_TOKEN is not set! Bot cannot start.")return# We must explicitly create a JobQueue and pass it to the builder
job_queue = JobQueue()
